              ...
              <td>{{ formattedDate(item.register_date_np) }}</td>
                <td><span class="label label-info"> {{ item.sample_collection.length }}</span>
                    <div v-if="item.latest_anc" title="Swab ID">
                      SID : <strong>{{ item.latest_anc.token }}</strong> <br> <!--#TODO check latest logic-->
                ...
                <td>
                  ...
                  <button v-if="checkPermission('lab-received') && checkAddReceivedView(item.latest_anc) && pcrAllowedOrganizationType.includes(hospitalType) && !checkPermission('antigen-result')" v-on:click="addReceivedInLab(item, removeItemOnSuccess)" title="Lab Received ( PCR / Antigen )">
