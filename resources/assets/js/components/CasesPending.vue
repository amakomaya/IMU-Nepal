            ...
            <tr slot-scope="{item, removeItemOnSuccess}">
              ...
              <td>{{ formattedDate(item.register_date_np) }}</td>
                ...
                <td>
                  ...
                  <button v-if="checkPermission('lab-received') && checkAddReceivedView(item.latest_anc) && pcrAllowedOrganizationType.includes(hospitalType)" v-on:click="addReceivedInLab(item, removeItemOnSuccess)" title="Lab Received ( PCR / Antigen )">
