      ...
      <tr slot-scope="{item}" v-if="latestLabResultNotNegative(item.latest_anc)">
        ...
        <td>
          ...
          <div v-if="role  == 'healthworker'">
            <button v-if="item.ancs.length == 0" v-on:click="aadSampleCollection(item.token)"
          ...
          <button v-if="checkForPositiveOnly(item.latest_anc)" v-on:click="viewConfirmReportForm(item)"
