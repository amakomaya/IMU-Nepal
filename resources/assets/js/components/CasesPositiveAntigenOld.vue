<template>
  <div>
    ...
    <filterable v-bind="filterable">
      ...
      <tr slot-scope="{item}">
        <td>
          <div v-if="checkForPositiveOnly(item.ancs_result)" title="Case ID">C ID : {{ item.case_id }}</div>
...
import Filterable from './FilterableOld.vue'
...
export default {
  ...
  data() {
    return {
      role: this.$userRole,
      filterable: {
        url: '/data/api/positive-patient-antigen-old',
        orderables: [
        ...
        filterGroups: [
          ...
          },
          {
            ...
            filters: [
              {title: 'Swab ID ', name: 'ancs_token', type: 'string'},
              {title: 'Swab Created At', name: 'ancs_created_at', type: 'datetime'}
            ]
          },
          {
            name: 'Lab Result',
  ...
  methods: {
    newLink() {
      window.location.href = window.location.protocol + '/admin/positive-patients-antigen';
    },
