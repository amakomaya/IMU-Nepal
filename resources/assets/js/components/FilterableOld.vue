    ...
    exportToExcelForDolphins() {
      ...
      let list=[];
        ...
        if(data.sex == '1') {
          ...
          exportableData.gender = gender;
          console.log(data.lab_tests_token);
          if(data.lab_tests_token){
            var formattedtoken = data.lab_tests_token.slice(1).split('-')[1] ? data.lab_tests_token.slice(1).split('-')[1] : data.lab_tests_token;
            exportableData.swab_id = formattedtoken;
        }
    ...
    exportToExcel() {
      if (confirm("Do you want to Download all records in excel ! ")) {
        ...
        this.collection.data.map(function(data, key) {
            ...
            var labresult = '';
            if(data.ancs_result == 2) {
                labresult = 'Pending';
            }else if(data.ancs_result == 3) {
                labresult = 'Positive';
            }else if(data.ancs_result == 4) {
                labresult = 'Negative';
            }else if(data.ancs_result == 9) {
                labresult = 'Received';
            }else{
                labresult = "Don't Know";
            }
            ...
            exportableData.age = data.age;
            exportableData.age_unit = age_unit;
            exportableData.gender = gender;
            exportableData.district = data.district_name;
            exportableData.municipality = data.municipality_name;
            exportableData.ward = data.ward;
            exportableData.current_hospital = data.healthpost_name;
            if(data.ancs_token){
            exportableData.swab_id = data.ancs_token;
            if(data.lab_tests_token){
                var formattedtoken = data.lab_tests_token.slice(1).split('-')[1] ? data.lab_tests_token.slice(1).split('-')[1] : data.lab_tests_token;
                exportableData.lab_id = formattedtoken;
            }
            exportableData.result = labresult;
            }
