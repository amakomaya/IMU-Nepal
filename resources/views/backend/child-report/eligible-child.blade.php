...
<style type="text/css">
    ...
    @endphp
        ...
        @php
         $months = ['Baishakh', 'Jestha', 'Asad','Shrawn','Bhadra','Aswin','Kartik','Mansir','Poush','Magh', 'Falgun', 'Chaitra' ];
        @endphp
        @foreach ($monthlyreport as $key => $dropoutRecord) 
        @php
            $to_date = date('Y-m-d', strtotime("+1 month", strtotime($to_date)));
            $calNepali = \App\Helpers\ViewHelper::convertEnglishToNepali($to_date); 
            list($nepaliYear, $nepaliMonth, $nepaliDay) = explode("-", $calNepali);
        @endphp
        <tr class="text-center">
            <td>{{$nepaliYear."/".$months[$nepaliMonth-1]}}</td>
            <td> </td>
            <td>
