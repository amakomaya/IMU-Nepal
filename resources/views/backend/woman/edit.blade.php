@extends('backend.woman.form')
    ...
    @section('action',route('woman.update', $data->id))

    @section('methodField',method_field('PUT'))

    @section ('name',!!old('name')? old('name') : $data->name)

    @section ('mool_darta_no',!!old('mool_darta_no')? old('mool_darta_no') : $data->mool_darta_no)

    @section ('sewa_darta_no',!!old('sewa_darta_no')? old('sewa_darta_no') : $data->sewa_darta_no)

    @section ('orc_darta_no',!!old('orc_darta_no')? old('orc_darta_no') : $data->orc_darta_no)

    @section ('phone',!!old('phone')? old('phone') : $data->phone)

    @section ('height',!!old('height')? old('height') : $data->height)

    @section ('age',!!old('age')? old('age') : $data->age)

    @section ('lmp_date_en',!!old('lmp_date_en')? old('lmp_date_en') : $lmp_date_en)
    ...
    @php($caste = !!old('caste')? old('caste') : $data->caste)

    @section ('husband_name',!!old('husband_name')? old('husband_name') : $data->husband_name)

    @php($anc_status = !!old('anc_status')? old('anc_status') : $data->anc_status)

    @php($delivery_status = !!old('delivery_status')? old('delivery_status') : $data->delivery_status)

    @php($pnc_status = !!old('pnc_status')? old('pnc_status') : $data->pnc_status)

    @php($labtest_status = !!old('labtest_status')? old('labtest_status') : $data->labtest_status)
