...
class OrganizationOld extends Model
{
	use LogsActivity;
    ...
    {
    	$healthpost = OrganizationOld::where('hp_code',$hp_code)->get()->first();
	...
	public static function generateHpCode($hp){
		...
		$healthpost = OrganizationOld::where('hp_code',$hpCode)->get()->first();
    	if(count($healthpost)>0){
	...
	public static function modelHealthpost($token){
        $model = OrganizationOld::where('token', $token)->get()->first();
        return $model;
	}
