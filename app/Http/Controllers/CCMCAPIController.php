...
class CCMCAPIController extends Controller
{
    public function getRecoveredCases()
    {
        ...
        $data = DB::table('payment_cases')->selectRaw('count(payment_cases.id) as total_cases')
            ...
            ->groupBy(DB::raw('d.district_name,is_death,gender,date(payment_cases.created_at)'))
            ->get();
        ...
        $data = DB::table('healthposts')
            ...
            ->leftJoin(
                DB::raw("( select
                    ...
                    hp_code from payment_cases where payment_cases.is_death is null group by hp_code) AS hp_bed_status"), 
                    'hp_bed_status.hp_code', 'healthposts.hp_code'
            )
