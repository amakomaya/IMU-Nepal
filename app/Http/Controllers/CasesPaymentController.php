...
class CasesPaymentController extends Controller
{
    ...
    public function report(Request $request){
        ...
        $organization = Organization::whereIn('org_code', $hpCodes);
        if($request->type){
        ...
        if ($request->has('selected_date')){
            ...
            $total = PaymentCase::leftjoin('organizations', 'organizations.org_code', '=', 'payment_cases.org_code')
                ->whereIn('payment_cases.org_code', $hpCodes);
            
            if($request->type){
                $total = $total->whereIn('organizations.hospital_type', $org_type);
            }
        ...
        if (auth()->user()->role === 'healthpost' || auth()->user()->role === 'healthworker'){
            $organization_is_oxygen_facility = Organization::where('org_code', $hpCodes);
            if($request->type){
