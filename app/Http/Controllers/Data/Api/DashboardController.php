...
//                ->where('vaccinated_status', '1')->count(),
    ...
    public function indexNew(Request $request)
    {
        ...
        $date_selected = $request->date_selected ?? Carbon::today();

        $antigen_positive = SampleCollection::whereIn('hp_code', $hpCodes)->where('service_for', '2')->where('result', '3')->active()->count();
        $antigen_negative = SampleCollection::whereIn('hp_code', $hpCodes)->where('service_for', '2')->where('result', '4')->active()->count();

        $pcr_positive = SampleCollection::whereIn('hp_code', $hpCodes)->where('service_for', '1')->where('result', '3')->active()->count();
        $pcr_positive = SampleCollection::whereIn('hp_code', $hpCodes)->where('service_for', '1')->where('result', '3')->active()->count();

        $hospital_admission = Organization::whereIn('hp_code', $hpCodes)->whereIn('hospital_type', [2,3])
        $hospital_active_cases = Organization::whereIn('hp_code', $hpCodes)->whereIn('hospital_type', [2,3])
        $hospital_discharge = Organization::whereIn('hp_code', $hpCodes)->whereIn('hospital_type', [2,3])
        PaymentCase::whereIn('hp_code', $hpCodes)->where('is_death', 1)

        $hospital_death = Organization::whereIn('hp_code', $hpCodes)->whereIn('hospital_type', [2,3])
        PaymentCase::whereIn('hp_code', $hpCodes)->where('is_death', 2)
