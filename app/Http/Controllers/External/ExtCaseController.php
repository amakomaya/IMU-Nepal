    ...
    public function storeV2(Request $request)
    {
        ...
        if (!empty($user)) {
            ...
            foreach ($data as $index=>$value) {
                ...
                if(!in_array(strval($value['lab_result']), ['3', '4'])) {
                  DB::rollback();
                  return response()->json(['message' => 'The data couldnot be uploaded due to following errors: Invalid Lab Result. Need value 3 For Positive, 4 For Negative. Error at index:'.$index, 'status' => 500 ]);
                }
                $isValidLabDate = $this->testValidEnDate($value['lab_test_date']);
                if(!$isValidLabDate) {
                  DB::rollback();
                  return response()->json(['message' => 'Invalid Lab Test Date. Please send a valid english date in YYYY-MM-DD format as mentioned in API documentation. Error at index:'.$index, 'status' => 500]);
                }
                $isValidSampleCollectedDate = $this->testValidEnDate($value['sample_collected_date']);
                if(!$isValidSampleCollectedDate) {
                  DB::rollback();
                  return response()->json(['message' => 'Invalid Sample Collected Date. Please send a valid english date in YYYY-MM-DD format as mentioned in API documentation. Error at index:'.$index, 'status' => 500 ]);
                }
                $isValidRegDate = $this->testValidEnDateTime($value['registered_at']);
                if(!$isValidRegDate) {
                  DB::rollback();
                  return response()->json(['message' => 'Invalid Registered Date. Please send a valid english date in YYYY-MM-DD format as mentioned in API documentation. Error at index:'.$index , 'status' => 500]);
                }
                $isValidLabReceivedDate = $this->testValidEnDate($value['lab_received_date']);
                if(!$isValidLabReceivedDate) {
                  DB::rollback();
                  return response()->json(['message' => 'Invalid Lab Received Date. Please send a valid english date in YYYY-MM-DD format as mentioned in API documentation. Error at index:'.$index, 'status' => 500]);
                }
                if(array_key_exists('imu_swab_id', $value) && $value['imu_swab_id']) {
                  $existingSampleCollection = SampleCollection::where('checked_by', $user->token)->where('token', $value['imu_swab_id'])->first();
                  if($existingSampleCollection) {
...
//                      unset($sample['reporting_date_np']);
                    ...
                    DB::rollback();
                    return response()->json(['message' => 'Error at index:'.$index.'.The data couldnot be uploaded due to following errors: \n '. $e->getMessage(), 'status' => $e->getCode()]);
                }
            }
            DB::commit();
            return response()->json(['message' => 'Data Successfully Sync', 'status' => 200]);
        }
...
//                $response['token'] = $row->token;
    ...
    public function getCaseDetailBySampleV2(Request $request)
    {
        $key = request()->getUser();
        $secret = request()->getPassword();
        $sample_token = $request->sample_token;
