...
class DownloadablePositiveListPoeByDate implements FromCollection, WithHeadings
{
    ...
    public function collection()
    {
        ...
        // $data = DB::raw("
        // SELECT suspected_cases.name, suspected_cases.age, genders.name as gender, suspected_cases.province_id, districts.district_name, municipalities.municipality_name, suspected_cases.ward, suspected_cases.tole, suspected_cases.emergency_contact_one, suspected_cases.emergency_contact_two , ancs.token SID, ancs.created_at, ancs.reporting_date_en FROM `ancs` LEFT JOIN suspected_cases on suspected_cases.token = ancs.woman_token LEFT JOIN districts ON suspected_cases.district_id = districts.id LEFT JOIN municipalities on suspected_cases.municipality_id = municipalities.id LEFT JOIN genders on suspected_cases.sex = genders.id LEFT JOIN healthposts h on h.hp_code = ancs.hp_code WHERE ((ancs.created_at like '2021-06-29%' and ancs.received_date_en is null) or ancs.reporting_date_en LIKE '2021-06-29%') and ancs.service_for = 2 AND ancs.result = 4 AND ancs.status = 1 and suspected_cases.name is not null ORDER BY `ancs`.`created_at` ASC
        // ")
